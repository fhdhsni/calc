export default function showhelp(helpObj) {
  ...
  const syntaxNodes = document.querySelectorAll(".syntax"); // these are duplicated with exitHelp, take care of them
  const examplesNode = document.querySelectorAll(".examples"); // these are duplicated with exitHelp, take care of them
  ...
  for (let i = 0; i < syntaxNodes.length; i += 1) { // these are duplicated with exitHelp, take care of them
    syntaxNodes[i].parentNode.removeChild(syntaxNodes[i]); // these are duplicated with exitHelp, take care of them
  } // these are duplicated with exitHelp, take care of them

  for (let i = 0; i < examplesNode.length; i += 1) { // these are duplicated with exitHelp, take care of them
    examplesNode[i].parentNode.removeChild(examplesNode[i]); // these are duplicated with exitHelp, take care of them
  } // these are duplicated with exitHelp, take care of them
